...
def get_info(sym):
    ...
    if polygon_key:
        try:
            data = poly_get(f"https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/{sym}?apiKey={polygon_key}")
            t    = data.get("ticker", {})
            day  = t.get("day", {})
            prev = t.get("prevDay", {})
    ...
    # 2. yfinance for name/sector/target/52w/earnings
    if HAS_YF:
        try:
            t = yf.Ticker(sym)
            ...
            try:
                ...
                if isinstance(cal, dict):
                    ...
                    if ed:
                        out["nextER"] = str(ed[0] if isinstance(ed, list) else ed)[:10]
            except: pass
        except: pass
    ...
    if polygon_key:
        try:
            for ct, store in [("call", calls), ("put", puts)]:
                url  = f"https://api.polygon.io/v3/snapshot/options/{sym}?expiration_date={exp}&contract_type={ct}&limit=100&apiKey={polygon_key}"
                data = poly_get(url)
                for r in data.get("results", []):
                    ...
                    if k > 0 and (bid > 0 or ask > 0):
                        store.append({"k": round(float(k), 1), "bid": round(bid, 2), "ask": round(ask, 2)})
            if calls or puts:
                calls.sort(key=lambda r: r["k"])
                puts.sort(key=lambda r: r["k"])
                return calls, puts
        except: pass
            ...
            def parse_rows(df):
                ...
                for r in df.to_dict("records"):
                    ...
                    if bid == 0 and ask == 0 and last > 0:
                        ...
                        ask = round(last * 1.05, 2)
                    if k > 0 and (bid > 0 or ask > 0):
                        rows.append({"k": k, "bid": round(bid, 2), "ask": round(ask, 2)})
                return rows
            col_c = [c for c in ["strike", "bid", "ask", "lastPrice"] if c in opt.calls.columns]
            col_p = [c for c in ["strike", "bid", "ask", "lastPrice"] if c in opt.puts.columns]
            calls = parse_rows(opt.calls[col_c].dropna(subset=["strike"]))
            puts  = parse_rows(opt.puts[col_p].dropna(subset=["strike"]))
            calls.sort(key=lambda r: r["k"])
            puts.sort(key=lambda r: r["k"])
            return calls, puts
        except: pass
...
def find_be(sk, bK, bP, sK, sP, cur):
    ...
    prev = pnl(sk, bK, bP, sK, sP, lo)
    for i in range(1, 601):
        p = lo + (hi - lo) * i / 600
        c = pnl(sk, bK, bP, sK, sP, p)
        if (prev <= 0 and c > 0) or (prev >= 0 and c < 0):
            f = abs(prev) / (abs(prev) + abs(c))
...
def make_svg(sk, bK, bP, sK, sP, cur):
    ...
    tks = "".join(
        f'<line x1="{cx(lo+(hi-lo)*i/5):.1f}" y1="{zY:.1f}" x2="{cx(lo+(hi-lo)*i/5):.1f}" y2="{zY+4:.1f}" stroke="#444" stroke-width=".8"/>'
        f'<text x="{cx(lo+(hi-lo)*i/5):.1f}" y="{zY+13:.1f}" fill="#444" font-size="7.5" text-anchor="middle">${lo+(hi-lo)*i/5:.0f}</text>'
        for i in range(6)
    )
    cl = (
        f'<line x1="{cx(cur):.1f}" y1="{pTop}" x2="{cx(cur):.1f}" y2="{H-pBot-lH}" stroke="#1F6FEB" stroke-width="1" stroke-dasharray="4 3" opacity=".5"/>'
        f'<text x="{cx(cur):.1f}" y="{pTop-4}" fill="#1F6FEB" font-size="9" text-anchor="middle" font-weight="600">現價 ${cur:.2f}</text>'
    ) if lo < cur < hi else ""
    dots = [(bK, True)] + ([(sK, False)] if sK else [])
    ds = ""
    for dk, buy in dots:
        if not (lo < dk < hi): continue
        dy  = cy(pnl(sk, bK, bP, sK, sP, dk))
        sx  = cx(dk)
        mc  = "#60A5FA" if buy else "#FB923C"
        lbl = ("買入" if buy else "賣出") + " $" + f"{dk:.0f}"
        ab  = dy > H * 0.45
        by2 = dy - 27 if ab else dy + 11
        ds += (
            f'<rect x="{sx-34:.1f}" y="{by2:.1f}" width="68" height="14" rx="3" fill="#0A0A0A" opacity=".92"/>'
            f'<text x="{sx:.1f}" y="{by2+11:.1f}" fill="{mc}" font-size="9" text-anchor="middle" font-weight="700">{lbl}</text>'
            f'<circle cx="{sx:.1f}" cy="{dy:.1f}" r="5" fill="{mc}"/>'
            f'<circle cx="{sx:.1f}" cy="{dy:.1f}" r="9" fill="{mc}" opacity=".18"/>'
        )
    be_s = ""
    if be and lo < be < hi:
        bx = cx(be)
        be_s = (
            f'<rect x="{bx-36:.1f}" y="{zY-32:.1f}" width="72" height="24" rx="4" fill="#0A0A0A" opacity=".92"/>'
            ...
            f'<circle cx="{bx:.1f}" cy="{zY:.1f}" r="5" fill="#F0B429"/>'
            f'<circle cx="{bx:.1f}" cy="{zY:.1f}" r="9" fill="#F0B429" opacity=".22"/>'
        )
    bY, bY2 = H - lH + 14, H - lH + 27
    pX1 = W - pX - 4 if prof_right else pX + 4
    pA1 = "end" if prof_right else "start"
    lX1 = pX + 4 if prof_right else W - pX - 4
    lA1 = "start" if prof_right else "end"
    mp  = "∞" if unlimited else f"+${maxV:.0f}"
    ml  = f"-${abs(minV):.0f}"
    return (
        f'<svg viewBox="0 0 {W} {H}" style="width:100%;height:{H}px;display:block;background:#0D1117;border-radius:14px;margin-top:10px">'
...
def make_ladder(sk, bK, bP, sK, sP, maxL, be, cur):
    ...
    ss = fmtm(step)
    h = (
        '<div style="background:#1C2128;border-radius:12px;overflow:hidden;margin-top:12px">'
    ...
    for r in rows:
        ...
        tags = (
            ('<span style="font-size:8px;background:#1A1000;color:#F0B429;padding:1px 5px;border-radius:100px;margin-left:4px;font-weight:700">平衡</span>' if r["nb"] else "") +
        ...
        h += (
            f'<tr style="background:{bg};border-bottom:1px solid #161B22">'
...
def add_fav(combo):
    favs = load_favs()
    combo["_id"] = f"{combo.get('ticker','')}_{len(favs)}"
    favs.insert(0, combo)
    save_favs(favs)
...
def compute(rows, cur, ticker, expiry):
    ...
    for i in range(len(valid)):
        for j in range(i + 1, len(valid)):
            ...
            if ba and sb and ba > 0:
                nc = ba - sb
                if nc > 0:
                    sp   = hi["strike"] - lo["strike"]
                    maxP = (sp - nc) * 100; maxL = nc * 100
                    if maxP > 0 and maxP / maxL >= 1 and cur and (lo["strike"] - cur) / cur <= 0.10:
            ...
            if ba and sb and ba > 0:
                ...
                if nc > 0:
                    sp   = hi["strike"] - lo["strike"]
                    maxP = (sp - nc) * 100; maxL = nc * 100
                    if maxP > 0 and maxP / maxL >= 1 and cur and (cur - hi["strike"]) / cur <= 0.10:
    ...
    for r in valid:
        ...
        if ba and ba > 0 and cur and abs(r["strike"] - cur) / cur <= 0.12:
            ...
            if maxP > 0:
                ...
                calls_list.append({"type":"call","strike":r["strike"],"premium":round(ba,2),"maxProfit":round(maxP),"maxLoss":round(maxL),"breakeven":round(be,2),"ror":round(ror),"rr":round(maxP/maxL,2),"_s":s})
    bulls.sort(key=lambda x: -x["_s"]); bears.sort(key=lambda x: -x["_s"]); calls_list.sort(key=lambda x: -x["_s"])
    ...
    def asgn(items):
        for it in items:
            ...
            it["pros"] = "、".join(pros) or "風險可控"
            it["cons"] = "、".join(cons) or "注意時間價值"
...
def render_combo(it, cur, typ, key=None):
    ...
    mp_s  = "∞" if unlimited else "$" + fmtm(mp)
    ror_s = "∞" if unlimited else f"{ror:.0f}%"
    html = (
    ...
    if key and st.button("⭐ 收藏", key=key):
        add_fav({**it,"ticker":st.session_state.get("scan_tkr",""),"sk":typ})
        st.success("已收藏！")

# ── header ────────────────────────────────────────────
st.markdown(
    '<div style="background:#161B22;padding:12px 16px 10px;border-bottom:1px solid #30363D;display:flex;align-items:center;gap:10px">'
    ...
    '<div><div style="font-size:9px;color:#8B949E;font-weight:600;letter-spacing:1px">OPTIONS</div>'
    '<div style="font-size:16px;font-weight:800;letter-spacing:-.5px">Strategy Pro</div></div></div>',
    unsafe_allow_html=True
)
...
with tab1:
    ...
    chip_s = (
        ...
        "border-radius:100px;padding:5px 12px;color:#8B949E;font-size:12px;"
        "font-weight:700;margin:2px;text-decoration:none;white-space:nowrap"
    )
    chips = "".join(f'<a href="?t={t}" style="{chip_s}">{t}</a>' for t in WATCHLIST)
    st.markdown(
        '<div style="overflow-x:auto;white-space:nowrap;padding:12px 16px 8px">' + chips + '</div>',
        unsafe_allow_html=True
    )
...
.srow{background:#1C2128;border:1px solid #30363D;border-radius:12px;padding:12px 14px;margin-bottom:10px;display:flex;align-items:center;gap:10px}
.srow select{flex:1;border:none;background:transparent;font-size:14px;padding:0}
.tip-btn{background:#0C2D6B;border:1px solid #1F6FEB;border-radius:8px;padding:6px 12px;color:#1F6FEB;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;flex-shrink:0}
...
.leg.sput{background:#3D2400;border:1.5px solid #F79000}
.ltitle{font-size:11px;font-weight:700;margin-bottom:8px}
.leg.buy .ltitle{color:#1F6FEB}.leg.sell .ltitle{color:#3FB950}.leg.sput .ltitle{color:#F79000}
.llabel{font-size:10px;color:#8B949E;margin-bottom:4px}
.prem-lbl{font-size:10px;color:#8B949E;margin-top:6px;margin-bottom:2px}
...
.qlbl{font-size:10px;color:#8B949E}
.qctrl{display:flex;align-items:center;gap:8px}
.qbtn{width:28px;height:28px;background:#21262D;border:1px solid #30363D;border-radius:7px;color:#E6EDF3;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.qval{font-size:14px;font-weight:700;min-width:20px;text-align:center}
.qunit{font-size:12px;color:#8B949E}
...
.mg{background:#0D4429;border:1.5px solid #1A6B36}
.mr{background:#4A1015;border:1.5px solid #8B1A1A}
.mgr{background:#1C2128;border:1.5px solid #30363D}
.mp{background:#2D1F52;border:1.5px solid #6E40C9}
.mlbl{font-size:9px;font-weight:700;margin-bottom:3px;letter-spacing:.5px}
.mg .mlbl{color:#86EFAC}.mr .mlbl{color:#FCA5A5}.mgr .mlbl{color:#8B949E}.mp .mlbl{color:#C4B5FD}
.mval{font-size:15px;font-weight:900;letter-spacing:-.5px}
.mg .mval{color:#4ADE80}.mr .mval{color:#F87171}.mgr .mval{color:#E6EDF3}.mp .mval{color:#A78BFA}
.msub{font-size:9px;font-weight:600;margin-top:1px}
.mg .msub{color:#4ADE80}.mr .msub{color:#F87171}
.rbox{background:#161B22;border:1px solid #30363D;border-radius:14px;padding:14px}
...
<div id="app"></div>
<script>
const DATA  = __DATAPLACEHOLDER__;
...
let sk="bull", tipOpen=false, result=null, bSel="", sSel="", bQty=1, sQty=1;
...
function makeLadder(sk,bK,bP,sK,sP,maxL,be){
  ...
  wrap.innerHTML='<div class="lhdr"><span class="lhl">損益對照表</span><span class="lhr">每 $'+ss+'</span></div><div class="lcols"><span class="lch">股價</span><span class="lch">損益/張</span><span class="lch">報酬率</span></div>';
  rows.forEach(r=>{
...
function render(){
  ...
  const bLbl=sk==="bull"?"買入 Call":sk==="bear"?"買入 Put":sk==="call"?"買入 Call":"買入 Put";
  const sLbl=sk==="bull"?"賣出 Call":"賣出 Put";
  const bCls=sk==="put"?"leg sput":"leg buy";
  const app=document.getElementById("app"); app.innerHTML="";
  // Strategy row
  const sr=document.createElement("div"); sr.className="srow";
  const ss=document.createElement("select"); ss.id="ss";
  Object.entries(STRATS).forEach(([k,v])=>{const o=document.createElement("option");o.value=k;o.textContent=v.label;if(k===sk)o.selected=true;ss.appendChild(o);});
  ss.onchange=e=>{sk=e.target.value;bSel="";sSel="";result=null;render();};
  const tb=document.createElement("button"); tb.className="tip-btn"; tb.textContent="ⓘ 說明";
  tb.onclick=()=>{tipOpen=!tipOpen;document.getElementById("tbox").style.display=tipOpen?"block":"none";};
  sr.appendChild(ss); sr.appendChild(tb); app.appendChild(sr);
  const tbox=document.createElement("div"); tbox.className="tip-box"; tbox.id="tbox";
  tbox.textContent=STRATS[sk].tip; tbox.style.display=tipOpen?"block":"none"; app.appendChild(tbox);
  // Legs
  const legsDiv=document.createElement("div"); legsDiv.className=isSingle?"":"legs"; app.appendChild(legsDiv);
  function makeLeg(arr,cls,title,selId,selVal,side,onChange){
    const card=document.createElement("div"); card.className=cls;
    const t=document.createElement("div"); t.className="ltitle"; t.textContent=title; card.appendChild(t);
    const lb=document.createElement("div"); lb.className="llabel"; lb.textContent="履約價 (Strike)"; card.appendChild(lb);
    const sel=document.createElement("select"); sel.id=selId;
    const o0=document.createElement("option"); o0.value=""; o0.textContent="點此選擇行權價"; sel.appendChild(o0);
    ...
    if(!selVal&&atm.k){setTimeout(()=>{const opts=Array.from(sel.options);const idx=opts.findIndex(o=>parseFloat(o.value)===atm.k);if(idx>0)sel.selectedIndex=idx;},0);}
    const row=arr.find(r=>r.k==selVal);
    if(row){
      const pl=document.createElement("div"); pl.className="prem-lbl"; pl.textContent=side+" 權利金"; card.appendChild(pl);
      const pv=document.createElement("div"); pv.className="prem-val"; pv.textContent="$"+(side==="Ask"?row.ask.toFixed(2):row.bid.toFixed(2)); card.appendChild(pv);
    } else {
      const ph=document.createElement("div"); ph.className="ph"; ph.textContent="選擇後顯示權利金"; card.appendChild(ph);
    }
    const qr=document.createElement("div"); qr.className="qrow";
    const ql=document.createElement("div"); ql.className="qlbl"; ql.textContent="數量";
    const qc=document.createElement("div"); qc.className="qctrl";
    const qm=document.createElement("button"); qm.className="qbtn"; qm.textContent="−";
    qm.onclick=()=>{if(selId==="selb"&&bQty>1)bQty--;else if(selId==="sels"&&sQty>1)sQty--;render();};
    const qv=document.createElement("div"); qv.className="qval"; qv.textContent=selId==="selb"?bQty:sQty;
    const qp=document.createElement("button"); qp.className="qbtn"; qp.textContent="+";
    qp.onclick=()=>{if(selId==="selb")bQty++;else sQty++;render();};
    const qu=document.createElement("div"); qu.className="qunit"; qu.textContent="張";
    qc.append(qm,qv,qp,qu); qr.append(ql,qc); card.appendChild(qr);
    return card;
  }
  legsDiv.appendChild(makeLeg(bArr,bCls,bLbl,"selb",bSel,"Ask",e=>{bSel=e.target.value;result=null;render();}));
  if(!isSingle){
    legsDiv.appendChild(makeLeg(sArr,"leg sell",sLbl,"sels",sSel,"Bid",e=>{sSel=e.target.value;result=null;render();}));
    const sw=document.createElement("button"); sw.className="swap-btn"; sw.textContent="⇄ 互換行權價";
    sw.onclick=()=>{const t=bSel;bSel=sSel;sSel=t;result=null;render();}; app.appendChild(sw);
  }
  // Calc button
  const cb=document.createElement("button"); cb.className="calc-btn";
  ...
  cb.onclick=doCalc; app.appendChild(cb);
  // Result
  if(result){
    ...
    const rb=document.createElement("div"); rb.className="rbox";
    ...
    [{cls:"mg",lbl:"最大獲利",val:mpS,sub:mpSub},{cls:"mr",lbl:"最大虧損",val:"-$"+maxL.toFixed(0),sub:"-"+((maxL/(maxL+(maxP||maxL)))*100).toFixed(0)+"%"},{cls:"mgr",lbl:"報酬率",val:rorS,sub:""},{cls:"mp",lbl:"損益平衡",val:beS,sub:""}].forEach(m=>{
      const md=document.createElement("div"); md.className="metric "+m.cls;
      md.innerHTML='<div class="mlbl">'+m.lbl+'</div><div class="mval">'+m.val+'</div>'+(m.sub?'<div class="msub">'+m.sub+'</div>':'');
      mg.appendChild(md);
    });
    rb.appendChild(mg);
    ...
    rb.appendChild(sb); app.appendChild(rb);
  }
}
function doCalc(){
  const bArr=getArr(),sArr=getSArr();
  ...
  const bK=bRow.k,bP=bRow.ask,sK=sRow?sRow.k:0,sP=sRow?sRow.bid:0;
  let maxP,maxL,be;
...
"""
        component_html = component_html.replace("__DATAPLACEHOLDER__", data_js)
        components.html(component_html, height=900, scrolling=True)

# ══════ TAB 2: AI 掃描 ══════
with tab2:
    st.markdown(
        '<div style="background:#0D4429;border:1px solid #1A6B36;border-radius:12px;padding:14px;margin:12px 16px">'
        '<div style="font-size:13px;color:#3FB950;font-weight:700;margin-bottom:4px">🤖 AI 智能分析</div>'
        '<div style="font-size:12px;color:#484F58;line-height:1.5">上傳1-3張期權鏈截圖，AI讀取數字，程式精算損益</div></div>',
        unsafe_allow_html=True
    )
    ...
    if uploaded:
        ...
        if st.button("🤖 AI 分析", key="scan"):
            if not api_key:
                st.error("請先設定 ANTHROPIC_API_KEY")
            else:
                with st.spinner("AI 讀取中..."):
                    try:
                        ...
                        prompt = (
                            f"讀取期權鏈截圖（{st.session_state.get('scan_tkr','未知')}）。{ph}"
    ...
    if st.session_state.get("scan_res"):
        ...
        if not allc:
            ...
            st.markdown(
                '<div style="display:flex;justify-content:space-between;align-items:flex-end;padding:6px 16px 12px;border-bottom:1px solid #30363D;margin-bottom:12px">'
                '<div><div style="font-size:20px;font-weight:900;letter-spacing:-.5px">' + tkr + '</div>'
                '<div style="font-size:11px;color:#8B949E">現價 $' + fmt(cur2) + '</div></div>'
                '<div style="text-align:right"><div style="font-size:10px;color:#8B949E;font-weight:700">最高報酬率</div>'
                '<div style="font-size:18px;font-weight:800;color:#3FB950">' + f"{mr:.0f}%" + '</div></div></div>',
                unsafe_allow_html=True
            )
            top3 = res.get("top3", [])
            if top3:
...
with tab3:
    ...
    if not favs:
        st.markdown(
            '<div style="background:#161B22;border-radius:12px;padding:28px;text-align:center;color:#8B949E;font-size:13px;margin:16px">'
            '尚無收藏<br><span style="font-size:11px">計算後點 ⭐ 收藏</span></div>',
            unsafe_allow_html=True
        )
    else:
        groups = {}
        ...
        for tk, items in groups.items():
            st.markdown(
                '<div style="font-size:15px;font-weight:800;margin:14px 16px 8px">📌 ' + tk +
                ' <span style="font-size:11px;color:#8B949E;font-weight:400">' + str(len(items)) + '個</span></div>',
                unsafe_allow_html=True
            )
            for f in items:
                typ = f.get("sk","")
                tn  = {"bull":"看漲價差","bear":"看跌價差","call":"單買Call","put":"單買Put"}.get(typ,"")
...
with tab4:
    h4 = (
        '<div style="padding:0 16px;color:#E6EDF3">'
        ...
        '<div style="font-size:13px;color:#8B949E;line-height:1.6">'
        '1. 點上方快捷或輸入股票代號<br>'
        '2. 選到期日<br>3. 選策略<br>4. 選行權價<br>5. 按計算損益'
        '</div></div></div>'
    )
